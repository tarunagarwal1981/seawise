...
# Database configuration, emission factors and vessel type mapping
DB_CONFIG = {
...
def show_cii_calculator():
    ...
    if 'draft_voyage_initialized' not in st.session_state:
        st.session_state.draft_voyage_initialized = False
    ...
    # Draft Voyage Logic
    if draft_voyage_clicked and not st.session_state.draft_voyage_initialized:
        st.session_state.draft_voyage_initialized = True
        ...
        try:
            ...
            st.error(f"Error in draft voyage calculation: {str(e)}")

    # Reset draft voyage state if calculate button is clicked
    ...
    with left_col:
        ...
        edited_df = st.data_editor(
            ...
            num_rows="dynamic",
            key=f"port_table_editor_{st.session_state.draft_voyage_initialized}",
            column_config={
